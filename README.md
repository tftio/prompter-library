...
- **Render instructions**: `prompter core.all` (or any other profile) prints the composed guidance.
...
4. Run `prompter validate` and spot-check key profiles (`core.all`, `backend.full`, `devops.full`).
5. Coordinate with the operator before introducing new categories or removing existing ones.

The goal is predictable, composable instruction sets that agents can consume without guesswork. Treat every fragment as production documentation.*** End Patch
